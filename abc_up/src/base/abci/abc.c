...
//    Cmd_CommandAdd( pAbc, "Verification", "dprove2",       Abc_CommandDProve2,          0 );
    ...
    Cmd_CommandAdd( pAbc, "Functional Dependency",      "check_fd",       Abc_CommandFD,       0 );
...
//Abc_NtkForEachObj(pNtkRes, pObj, c)
...
int Abc_CommandFD( Abc_Frame_t * pAbc, int argc, char ** argv )
{
	...
	to_be_deleted = malloc(numOuts * sizeof(int));
...
//find the index of the min value in fd_result
	...
	for(i = 0; i < numOuts; i++)
	{
		if ( fd_result[i] >= 0 )
			{	
				if ( fd_result[i] < min_value)
				{
					min_value = fd_result[i];
					indexCount = i;
				}
			}
	}
	...
	printf("The index of the min value is : %d\n", indexCount);	

//fill to_be_deleted array with 0's
...
//printing to_be_deleted array
	printf("\n");
	printf("to_be_deleted array:\n");
	printf("\n");
	for(i = 0; i < numOuts; i++)
	{
		printf("%d\t", to_be_deleted[i]);
	}
	printf("\n");
